...
export const Route = createFileRoute("/work")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Selected logo, brand identity, UI/UX and front-end projects by Nairobi designer ondwariobiko." },
      { property: "og:title", content: "Selected Work — ondwariobiko" },
      { property: "og:description", content: "Logos, brand systems and digital products designed in Nairobi." },
    ],
  }),
  component: WorkPage,
});
        ...
        <div className="max-w-[1400px] mx-auto border-t border-white/15">
          {projects.map((p) => (
            <article key={p.title} className="group relative border-b border-white/15 py-8 md:py-10 grid grid-cols-12 items-center gap-4 cursor-pointer">
              ...
              <h2 className="col-span-10 md:col-span-6 font-display uppercase tracking-tighter text-4xl md:text-7xl leading-none transition-all duration-500 group-hover:translate-x-4 group-hover:text-neon">{p.title}</h2>
